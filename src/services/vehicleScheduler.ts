...
export class VehicleSchedulerService {
  async solve() {
    ...
    try {
      ...
      // Parse depot data - select depot with highest capacity
      ...
      // Solve using 0/1 knapsack algorithm
      ...
      if (maxCapacity <= 0 || items.length === 0) {
        return {
          ...
          totalImpact: 0,
          selectedTaskIDs: [],
          debug: 'Invalid capacity or no tasks'
        };
      }

      // Dynamic programming table: dp[capacity] = max impact
